...
def readChoice(game_option, dif_level=1):
    board = Sudoku()
    if(game_option == '1'):
        question_board_code = board.generateSudokuCode(int(dif_level)) # generates a medium level sudoku
        print()
        print('Generating...')
        print()
        ...
        solved_board_code = Sudoku(code).solveSudokuToCode() # solves a hard level sudoku
        print(solved_board_code)